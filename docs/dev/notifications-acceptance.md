...
| 16 | passed 2026-09-16 | Brave running with no Merlin tab: push in the tray, the click opened a Merlin tab on the window. Two lessons on the way: a desktop push needs the browser process running (the first attempt with Brave quit received nothing), and on Linux the click must reach the browser as the notification's default action, which the notification daemon decides (dunst closes on left click by default). Also found here: Claude Code 2.1.273 ran the state hooks with a trimmed environment, fixed by `agent-window.sh`. |
