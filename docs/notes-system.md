...
Reference documentation for Merlin's 3-layer notes architecture: user profile, daily logs, and knowledge base.

## Overview

Merlin's notes live in the notes directory (resolve with `merlin config notes-dir`, default `~/.merlin/notes/`) and are organized into three layers with increasing detail:
...
| `<notes-dir>/kb/_index.md` | KB entry point and tag taxonomy |
