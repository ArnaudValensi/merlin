...
The `/cron` page manages Merlin's built-in scheduler. It starts with the
...
![Cron jobs list on a phone](cron/phone-cron-jobs.jpg)

The page has three tabs: Jobs, Performance, and Logs. The same jobs can
also be managed from the terminal with `merlin cron`.
...
![Cron performance dashboard](cron/phone-cron-performance.jpg)
...
`merlin cron` covers the same jobs: add, list, get, enable, disable,
remove, trigger, history. `merlin cron --help` has the full reference.
One example:

```bash
merlin cron add --schedule "0 7 * * *" \
...
timezones are set in the web UI. `merlin cron trigger <id>` runs a job
immediately, bypassing the schedule; `merlin cron history` shows recent
runs.

## Continue a run on Discord

A cron report on Discord is not a dead end. Messages posted in a thread
...
- **A new job did not run right away**: the scheduler registers a job the
  ...
  Save & run now, Run Now, or `merlin cron trigger` to fire it now.
...
- **Red banner on /cron**: the scheduler crashed in the last 24 hours.
  Check the logs, then dismiss.
- **"Job already running (locked)"**: a per-job lock prevents overlapping
  runs; the scheduler skips a job that is mid-run and `merlin cron
