...
const SyncStatus = {
    async check() {
        try {
            ...
            const text = document.getElementById('sync-conflict-text');
            if (!banner || !text) return;
            if (data.has_conflicts) {
                const files = data.conflicted_files.map(f => f).join(', ');
                text.textContent = `Merge conflict in ${files} — edit to resolve`;
...
const NoteView = {
    ...
    _resolveLinks(html) {
        ...
        // Second: media/ links → serve from notes root (not relative to current note)
        html = html.replace(
            /href="(media\/[^"]+)"/g,
    ...
    _resolveImages(html) {
        ...
        return html.replace(
            /src="(?!https?:\/\/|data:)([^"]+)"/g,
            (match, path) => {
                // media/ paths are always relative to notes root, not current note dir
