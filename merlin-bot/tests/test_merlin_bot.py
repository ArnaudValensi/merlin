...
class TestOnMessage:
    ...
    def test_session_id_passed_to_invoke(self, mock_invoke):
        """Session ID is passed to invoke for history loading."""
        mock_invoke.return_value = MagicMock(exit_code=0, session_id="s1", stderr="")
        ...
        kwargs = mock_invoke.call_args[1]
        assert kwargs["session_id"] == "some-session"

    @patch("merlin_bot.invoke")
    def test_single_invoke_call(self, mock_invoke):
        """No resume-first retry — single invoke call per message."""
        mock_invoke.return_value = MagicMock(exit_code=0, session_id="s1", stderr="")
        session_registry.set_thread_session("88888888", "some-session")
