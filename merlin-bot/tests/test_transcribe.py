...
class TestSaaSBackend:
    @patch("httpx.post")
    def test_calls_saas_api(self, mock_post):
        mock_response = MagicMock()
        mock_response.json.return_value = {"text": "saas result"}
        mock_response.raise_for_status = MagicMock()
        mock_post.return_value = mock_response

        with patch("builtins.open", mock_open(read_data=b"audio data")):
            result = transcribe._transcribe_saas("/tmp/test.ogg", "en", "test-token")

        assert result == "saas result"
        mock_post.assert_called_once()
        call_kwargs = mock_post.call_args
        assert "/api/transcribe" in call_kwargs.args[0]
...
class TestBackendSelection:
    ...
    def test_saas_takes_priority_over_openai(self, mock_saas):
        result = transcribe.transcribe("/tmp/test.ogg")
        assert result == "saas result"
        mock_saas.assert_called_once_with("/tmp/test.ogg", "fr", "test-token")
