...
All dependencies are declared in `pyproject.toml` and pinned by `uv.lock` for reproducible builds. Do NOT use PEP 723 inline metadata in application code.
...
**Checklist:**
- [ ] New dependencies added to `pyproject.toml`, not inline in `.py` files
- [ ] `uv lock` run after adding/removing dependencies (updates `uv.lock`)
- [ ] `uv.lock` committed alongside `pyproject.toml` changes
- [ ] Dependencies match what's actually imported (no dead deps, no missing deps)

**Anti-patterns:**
- Adding PEP 723 `# /// script` metadata to application code — use `pyproject.toml` instead
- Forgetting to run `uv lock` after changing deps — lock file will be stale
- Committing `pyproject.toml` changes without the updated `uv.lock`

### Exception: Standalone Scripts

Claude Code skills/hooks (`.claude/skills/`, `.claude/hooks/`) may still use PEP 723 inline metadata since they run independently outside the project context.
