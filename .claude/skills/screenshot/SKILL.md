...
# No auth (local-only mode without password)
...
- **No `--pass`** → no auth. For local-only mode where `DASHBOARD_PASS` is not set.
...
2. Run screenshots: `uv run .claude/skills/screenshot/screenshot.py --all <base-url> --pass <pass>`
...
- **Output naming**: use `<feature>-<viewport>.png` (e.g. `modal-desktop.png`) so files sort nicely alongside CLI-generated screenshots
