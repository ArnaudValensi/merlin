...
logger = logging.getLogger("merlin.claude")
...
def _load_user_context() -> str | None:
    ...
    # New path: ~/.merlin/user.md
    content = _load_file_content(paths.merlin_home() / "user.md")
    if content:
        return f"# User Memory\n\n{content}"

    # Fallback: legacy path ~/.merlin/notes/user.md
...
def _load_personality() -> str | None:
    ...
    # New path: ~/.merlin/personality.md
    content = _load_file_content(paths.merlin_home() / "personality.md")
    if content:
        return content

    # Fallback: legacy path ~/.merlin/merlin-bot/personality.md
...
def invoke_claude(
    ...
    *,
    ...
    Args:
        ...
        extra_system_prompts: List of file paths whose content is appended to
    ...
    # Auto-inject personality and user context
    for extra in (_load_personality(), _load_user_context()):
        if extra:
    ...
    # Append extra system prompts from files
...
def main() -> None:
    """CLI entry point: uv run claude.py [options] PROMPT"""
    import argparse
...
Examples:
  # Simple invocation
  uv run lib/claude.py "What is 2+2?"
...
Logging:
  Engine events logged to: logs/engine-log.jsonl
