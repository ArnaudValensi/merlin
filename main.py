...
def api_toggle_extension(ext_id: str, _auth=Depends(require_auth)):
    ...
    if not new_enabled and info.disable is not None:
        try:
            info.disable()
        except Exception as exc:
            logger.warning("Extension %s could not be disabled safely: %s", ext_id, exc)
            return JSONResponse(
                {"detail": "Extension cleanup failed; it remains enabled"},
                status_code=500,
            )
...
# --- Built-in: Agent activity timeline ---
def _load_timeline():
    import timeline
