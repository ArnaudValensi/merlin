...
The comparison model (single commit, range, branch, working tree) lives in
``compare.py`` on top of the helpers here.
"""
...
def _run_git(*args: str, repo_dir: Path, check: bool = True) -> str:
...
def get_commits(
    ...
    output = _run_git(*args, repo_dir=repo_dir, check=False)
