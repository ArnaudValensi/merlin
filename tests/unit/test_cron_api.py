...
class TestCreateJob:
    def test_create_returns_201(self, client):
        """POST /api/cron/jobs creates a job and returns 201."""
    ...
    def test_create_invalid_schedule_returns_422(self, client):
        ...
        resp = client.post("/api/cron/jobs", json=_sample_job(schedule="invalid"))
...
class TestListJobs:
    def test_list_empty(self, client):
        """GET /api/cron/jobs returns empty list when no jobs."""
        resp = client.get("/api/cron/jobs")
        assert resp.status_code == 200
        assert resp.json() == []

    def test_list_returns_all(self, client):
        """GET /api/cron/jobs returns all created jobs."""
        client.post("/api/cron/jobs", json=_sample_job(id="job-a"))
        client.post("/api/cron/jobs", json=_sample_job(id="job-b"))
        resp = client.get("/api/cron/jobs")
        assert resp.status_code == 200
        data = resp.json()
        assert len(data) == 2
...
# ---------------------------------------------------------------------------
# GET /api/cron/jobs/{job_id} — get single
# ---------------------------------------------------------------------------


class TestGetJob:
    def test_get_existing(self, client):
        """GET /api/cron/jobs/{id} returns job with history."""
        client.post("/api/cron/jobs", json=_sample_job())
        resp = client.get("/api/cron/jobs/test-job")
        assert resp.status_code == 200
        data = resp.json()
        assert data["id"] == "test-job"
    ...
    def test_get_nonexistent_returns_404(self, client):
        """GET /api/cron/jobs/missing returns 404."""
        resp = client.get("/api/cron/jobs/missing")
        assert resp.status_code == 404


# ---------------------------------------------------------------------------
# PUT /api/cron/jobs/{job_id} — update
# ---------------------------------------------------------------------------
...
class TestUpdateJob:
    ...
    def test_update_nonexistent_returns_404(self, client):
        ...
        resp = client.put("/api/cron/jobs/missing", json={"description": "test"})
        assert resp.status_code == 404

    def test_update_invalid_schedule(self, client):
        """PUT with invalid schedule returns 422."""
        client.post("/api/cron/jobs", json=_sample_job())
        resp = client.put("/api/cron/jobs/test-job", json={"schedule": "bad"})
        assert resp.status_code == 422
...
class TestDeleteJob:
    def test_delete_returns_204(self, client):
        """DELETE /api/cron/jobs/{id} returns 204."""
        client.post("/api/cron/jobs", json=_sample_job())
        resp = client.delete("/api/cron/jobs/test-job")
        assert resp.status_code == 204

        # Confirm gone
        resp2 = client.get("/api/cron/jobs/test-job")
    ...
    def test_delete_cleans_state(self, client, _isolated_cron_dirs):
        ...
        client.post("/api/cron/jobs", json=_sample_job())
        cron_state.STATE_DIR.mkdir(parents=True, exist_ok=True)
        (cron_state.STATE_DIR / "test-job").write_text("2026-01-01T00:00:00+00:00")
        cron_state.LOCKS_DIR.mkdir(parents=True, exist_ok=True)
        (cron_state.LOCKS_DIR / "test-job.lock").write_text("")

        resp = client.delete("/api/cron/jobs/test-job")
        assert resp.status_code == 204
        assert not (cron_state.STATE_DIR / "test-job").exists()
        assert not (cron_state.LOCKS_DIR / "test-job.lock").exists()


# ---------------------------------------------------------------------------
# POST /api/cron/jobs/{job_id}/toggle — toggle enabled
# ---------------------------------------------------------------------------


class TestToggleJob:
    def test_toggle_flips_enabled(self, client):
        """POST /api/cron/jobs/{id}/toggle flips enabled state."""
        client.post("/api/cron/jobs", json=_sample_job(enabled=True))
        resp = client.post("/api/cron/jobs/test-job/toggle")
        assert resp.status_code == 200
        assert resp.json()["enabled"] is False

        # Toggle back
        resp2 = client.post("/api/cron/jobs/test-job/toggle")
        assert resp2.status_code == 200
        assert resp2.json()["enabled"] is True

    def test_toggle_nonexistent_returns_404(self, client):
        """POST toggle nonexistent job returns 404."""
        resp = client.post("/api/cron/jobs/missing/toggle")
        assert resp.status_code == 404


# ---------------------------------------------------------------------------
# POST /api/cron/jobs/{job_id}/run — trigger run
# ---------------------------------------------------------------------------
