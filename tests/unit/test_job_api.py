...
class TestCreateJob:
    def test_create_returns_201(self, client):
        """POST /api/jobs/jobs creates a job and returns 201."""
        resp = client.post("/api/jobs/jobs", json=_sample_job())
    ...
    def test_create_invalid_schedule_returns_422(self, client):
        ...
        resp = client.post("/api/jobs/jobs", json=_sample_job(schedule="invalid"))
    ...
    def test_create_with_webhook_block(self, client, _isolated_job_dirs):
        ...
        resp = client.post(
            "/api/jobs/jobs", json=_sample_job(webhook={"secret": "whk_test"})
        )
        assert resp.status_code == 201
        import json as json_mod
    ...
    def test_create_invalid_report_mode(self, client):
        ...
        resp = client.post("/api/jobs/jobs", json=_sample_job(report_mode="verbose"))
        assert resp.status_code == 422
    ...
    def test_create_persists_timezone(self, client):
        ...
        resp = client.post("/api/jobs/jobs", json=_sample_job(timezone="Europe/Paris"))
        assert resp.status_code == 201
    ...
    def test_create_invalid_timezone_returns_422(self, client):
        ...
        resp = client.post("/api/jobs/jobs", json=_sample_job(timezone="Not/AZone"))
        assert resp.status_code == 422
...
class TestListJobs:
    def test_list_empty(self, client):
        """GET /api/jobs/jobs returns empty list when no jobs."""
        resp = client.get("/api/jobs/jobs")
        assert resp.status_code == 200
        assert resp.json() == []

    def test_list_returns_all(self, client):
        """GET /api/jobs/jobs returns all created jobs."""
        client.post("/api/jobs/jobs", json=_sample_job(id="job-a"))
        client.post("/api/jobs/jobs", json=_sample_job(id="job-b"))
        resp = client.get("/api/jobs/jobs")
        assert resp.status_code == 200
        data = resp.json()
        assert len(data) == 2
...
# ---------------------------------------------------------------------------
# GET /api/jobs/jobs/{job_id} — get single
# ---------------------------------------------------------------------------


class TestGetJob:
    def test_get_existing(self, client):
        """GET /api/jobs/jobs/{id} returns job with history."""
        client.post("/api/jobs/jobs", json=_sample_job())
        resp = client.get("/api/jobs/jobs/test-job")
        assert resp.status_code == 200
        data = resp.json()
        assert data["id"] == "test-job"
    ...
    def test_get_nonexistent_returns_404(self, client):
        """GET /api/jobs/jobs/missing returns 404."""
        resp = client.get("/api/jobs/jobs/missing")
        assert resp.status_code == 404


# ---------------------------------------------------------------------------
# PUT /api/jobs/jobs/{job_id} — update
# ---------------------------------------------------------------------------


class TestUpdateJob:
    def test_update_fields(self, client):
        """PUT /api/jobs/jobs/{id} merges only provided fields."""
        client.post("/api/jobs/jobs", json=_sample_job())
        resp = client.put(
            "/api/jobs/jobs/test-job",
            json={
    ...
    def test_update_nonexistent_returns_404(self, client):
        ...
        resp = client.put("/api/jobs/jobs/missing", json={"description": "test"})
        assert resp.status_code == 404
    ...
    def test_update_empty_schedule_removes_trigger(self, client, _isolated_job_dirs):
        ...
        client.post("/api/jobs/jobs", json=_sample_job())
        resp = client.put("/api/jobs/jobs/test-job", json={"schedule": ""})
        assert resp.status_code == 200
        assert resp.json()["next_run"] is None
        import json as json_mod
    ...
    def test_update_clears_working_dir(self, client, _isolated_job_dirs):
        ...
        client.post("/api/jobs/jobs", json=_sample_job(working_dir="/old/path"))
        assert (
        ...
        resp = client.put("/api/jobs/jobs/test-job", json={"working_dir": ""})
        assert resp.status_code == 200
        stored = json_mod.loads((_isolated_job_dirs / "test-job.json").read_text())
    ...
    def test_create_empty_working_dir_stored_absent(self, client, _isolated_job_dirs):
        ...
        client.post("/api/jobs/jobs", json=_sample_job(working_dir=""))
        stored = json_mod.loads((_isolated_job_dirs / "test-job.json").read_text())
    ...
    def test_update_invalid_schedule(self, client):
        ...
        client.post("/api/jobs/jobs", json=_sample_job())
        resp = client.put("/api/jobs/jobs/test-job", json={"schedule": "bad"})
        assert resp.status_code == 422

    def test_update_empty_prompt_rejected(self, client):
        """PUT with empty prompt returns 422."""
        client.post("/api/jobs/jobs", json=_sample_job())
        resp = client.put("/api/jobs/jobs/test-job", json={"prompt": ""})
        assert resp.status_code == 422
    ...
    def test_update_type_switch_without_command_rejected(self, client):
        ...
        client.post("/api/jobs/jobs", json=_sample_job())
        resp = client.put("/api/jobs/jobs/test-job", json={"type": "command"})
        assert resp.status_code == 422
    ...
    def test_update_type_switch_to_prompt_without_prompt_rejected(self, client):
        ...
        resp = client.put("/api/jobs/jobs/cmd-only", json={"type": "prompt"})
        assert resp.status_code == 422
    ...
    def test_update_command_field(self, client):
        ...
        resp = client.put("/api/jobs/jobs/cmd-job", json={"command": "echo new"})
        assert resp.status_code == 200
        assert resp.json()["command"] == "echo new"


# ---------------------------------------------------------------------------
# DELETE /api/jobs/jobs/{job_id} — delete
# ---------------------------------------------------------------------------


class TestDeleteJob:
    def test_delete_returns_204(self, client):
        """DELETE /api/jobs/jobs/{id} returns 204."""
        client.post("/api/jobs/jobs", json=_sample_job())
        resp = client.delete("/api/jobs/jobs/test-job")
        assert resp.status_code == 204

        # Confirm gone
        resp2 = client.get("/api/jobs/jobs/test-job")
        assert resp2.status_code == 404

    def test_delete_nonexistent_returns_404(self, client):
        """DELETE nonexistent job returns 404."""
        resp = client.delete("/api/jobs/jobs/missing")
        assert resp.status_code == 404
...
# ---------------------------------------------------------------------------
# POST /api/jobs/jobs/{job_id}/toggle — toggle enabled
# ---------------------------------------------------------------------------


class TestToggleJob:
    def test_toggle_flips_enabled(self, client):
        """POST /api/jobs/jobs/{id}/toggle flips enabled state."""
        client.post("/api/jobs/jobs", json=_sample_job(enabled=True))
        resp = client.post("/api/jobs/jobs/test-job/toggle")
        assert resp.status_code == 200
        assert resp.json()["enabled"] is False

        # Toggle back
        resp2 = client.post("/api/jobs/jobs/test-job/toggle")
        assert resp2.status_code == 200
        assert resp2.json()["enabled"] is True

    def test_toggle_nonexistent_returns_404(self, client):
        """POST toggle nonexistent job returns 404."""
        resp = client.post("/api/jobs/jobs/missing/toggle")
        assert resp.status_code == 404


# ---------------------------------------------------------------------------
# POST /api/jobs/jobs/{job_id}/run — trigger run
# ---------------------------------------------------------------------------


class TestRunJob:
    def test_run_returns_202(self, client):
        """POST /api/jobs/jobs/{id}/run triggers subprocess and returns 202."""
        client.post("/api/jobs/jobs", json=_sample_job())
        with patch(
            "asyncio.create_subprocess_exec", new_callable=AsyncMock
        ) as mock_exec:
            resp = client.post("/api/jobs/jobs/test-job/run")
    ...
    def test_run_nonexistent_returns_404(self, client):
        ...
        resp = client.post("/api/jobs/jobs/missing/run")
        assert resp.status_code == 404
