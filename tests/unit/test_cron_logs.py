...
def _isolated_cron_dirs(tmp_path):
    """Patch cron.manage, cron.state, and cron.logs to use temp directories."""
...
def log_dir(tmp_path):
    """Provide a temp cron-logs directory and patch logs module to use it."""
    d = tmp_path / "cron-logs"
    d.mkdir()
...
class TestLogEndpoints:
    def test_get_logs_returns_list(self, client, log_dir):
        """GET /api/cron/jobs/{id}/logs returns log list."""
        # Create a job first
    ...
    def test_get_logs_nonexistent_job_returns_404(self, client):
        ...
        resp = client.get("/api/cron/jobs/nonexistent/logs")
    ...
    def test_get_single_log_not_found_returns_404(self, client):
        ...
        client.post("/api/cron/jobs", json=_sample_job())
        resp = client.get("/api/cron/jobs/test-job/logs/2099-01-01T00:00:00+00:00")
        assert resp.status_code == 404

    def test_delete_job_also_deletes_logs(self, client, log_dir):
        """DELETE /api/cron/jobs/{id} also removes execution logs."""
        client.post("/api/cron/jobs", json=_sample_job())
