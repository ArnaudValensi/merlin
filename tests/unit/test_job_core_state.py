"""Tests for job.state — state, history, and locking."""
...
def temp_jobs_dir(tmp_path):
    """Fixture that patches job.state to use a temporary directory."""
    from job import state as job_state

    original_state_dir = job_state.STATE_DIR
    original_locks_dir = job_state.LOCKS_DIR
    original_history = job_state.HISTORY_FILE
    ...
    job_state.STATE_DIR = original_state_dir
    job_state.LOCKS_DIR = original_locks_dir
    job_state.HISTORY_FILE = original_history
...
class TestState:
    ...
    def test_set_last_run_defaults_to_now(self, temp_jobs_dir):
    ...
    def test_per_job_isolation(self, temp_jobs_dir):
    ...
    def test_state_file_is_plain_text(self, temp_jobs_dir):
        """State files are plain ISO timestamps, not JSON."""
        from job.state import STATE_DIR, set_last_run
    ...
    def test_get_last_run_handles_corrupt_file(self, temp_jobs_dir):
        """get_last_run returns None for corrupt state file."""
        from job.state import STATE_DIR, get_last_run

        STATE_DIR.mkdir(parents=True, exist_ok=True)
        (STATE_DIR / "bad-job").write_text("not a timestamp")
        assert get_last_run("bad-job") is None

    def test_get_last_run_handles_empty_file(self, temp_jobs_dir):
...
class TestJobLocking:
    ...
    def test_acquire_and_release_lock(self, temp_jobs_dir):
    ...
    def test_lock_prevents_second_acquisition(self, temp_jobs_dir):
    ...
    def test_different_jobs_lock_independently(self, temp_jobs_dir):
    ...
    def test_lock_blocks_across_threads(self, temp_jobs_dir):
...
class TestHistory:
    ...
    def test_read_history_empty(self, temp_jobs_dir):
        """read_history returns empty dict when no history file exists."""
        from job.state import read_history

        assert read_history() == {}

    def test_append_history_creates_entry(self, temp_jobs_dir):
        """append_history creates a new entry for a job."""
        from job.state import append_history, read_history
    ...
    def test_append_history_multiple_entries(self, temp_jobs_dir):
        """append_history accumulates entries."""
        from job.state import append_history, get_history
    ...
    def test_history_rolling_limit(self, temp_jobs_dir):
        """append_history enforces rolling limit of 100."""
        from job.state import HISTORY_LIMIT, append_history, get_history
    ...
    def test_get_history_with_limit(self, temp_jobs_dir):
    ...
    def test_get_history_unknown_job(self, temp_jobs_dir):
        """get_history returns empty list for unknown job."""
        from job.state import get_history

        assert get_history("unknown-job") == []

    def test_get_all_history(self, temp_jobs_dir):
        """get_all_history returns history for all jobs."""
        from job.state import append_history, get_all_history
    ...
    def test_read_history_handles_invalid_json(self, temp_jobs_dir):
        """read_history returns empty dict for invalid JSON."""
        from job import state as job_state

        job_state.HISTORY_FILE.write_text("not valid json")
        from job.state import read_history

        assert read_history() == {}

    def test_concurrent_history_appends(self, temp_jobs_dir):
