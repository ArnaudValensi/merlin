"""Tests for the timezone-correct, enriched POST /api/cron/validate-schedule."""
...
def test_valid_returns_human_timezone_and_runs(client, monkeypatch):
    ...
    resp = client.post("/api/cron/validate-schedule", json={"schedule": "0 9 * * *"})
...
def test_invalid_schedule(client):
    resp = client.post("/api/cron/validate-schedule", json={"schedule": "not a cron"})
    assert resp.status_code == 200
    data = resp.json()
