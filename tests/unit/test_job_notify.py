"""Tests for job.notify — notification with graceful fallback to Discord."""
...
class TestNotifyJobResult:
    ...
    def test_bot_loaded_channel_configured(self):
        ...
        notify_job_result("daily-check", _sample_job(), _sample_result(), registry)
    ...
    def test_session_id_passed_to_notify(self):
        ...
        notify_job_result("daily-check", _sample_job(), result, registry)
    ...
    def test_notify_raises_exception_logged_no_crash(self, caplog):
        ...
        with caplog.at_level(logging.DEBUG, logger="merlin.job"):
            notify_job_result("daily-check", _sample_job(), _sample_result(), registry)
    ...
    def test_per_job_discord_channel_overrides_global(self):
        ...
        notify_job_result("daily-check", job, _sample_result(), registry)
    ...
    def test_silent_mode_skips_on_success(self):
        ...
        notify_job_result("daily-check", job, _sample_result(exit_code=0), registry)
        mod.notify.assert_not_called()
    ...
    def test_silent_mode_notifies_on_error(self):
        ...
        job = _sample_job(report_mode="silent")
        notify_job_result("daily-check", job, _sample_result(exit_code=1), registry)
        mod.notify.assert_called_once()
    ...
    def test_outer_exception_caught_and_logged(self, caplog):
        """Even if _do_notify raises unexpectedly, notify_job_result never raises."""
