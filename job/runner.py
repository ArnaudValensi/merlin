"""
Job dispatcher — runs every minute via the scheduler in job/__init__.py.

Reads job files from jobs/, checks if each is due, and executes via Claude.
...
def session_id_for_job(job_id: str) -> str:
    ...
    return str(uuid.uuid5(uuid.NAMESPACE_DNS, f"job-{job_id}"))
...
def _execute_job(job_id: str, job: dict, *, emit_result: bool = True):
    ...
    try:
        from job.logs import cleanup_logs, write_log
    ...
    # pick it up and send Discord notifications via notify_job_result().
    if emit_result:
        try:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
How it works:
  1. Reads all job files from jobs/*.json
...
Manual execution:
  ...
  uv run job/runner.py --job daily-python-check

Job file format (jobs/<job-id>.json):
  {
...
Related commands:
  merlin job --help              # Manage jobs (add, list, enable, etc.)
...
Logs:
  ...
  jobs/.state/               # Per-job last run timestamps
  jobs/.history.json         # Run history
""",
